-- Fix migrate_profile_auth_id: create the new Profile row before updating FKs
-- (assignedBy/grantor FKs require the target Profile id to exist).
...
BEGIN
  ...
  -- Free the unique email slot while keeping the old row for copying.
  ...
  UPDATE public."IntakeFormConfig" SET "createdById" = new_id WHERE "createdById" = old_id;
