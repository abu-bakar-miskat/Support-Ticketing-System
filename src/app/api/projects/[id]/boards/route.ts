import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/db";
import { requireAuth } from "@/lib/auth";
import { projectInScope } from "@/lib/dept-scope";
import { canManageProjectBoards } from "@/lib/project-permissions";
import { broadcastProjectBoardsChange } from "@/lib/project-boards-broadcast";
import {
  memberTeamIdsFromProject,
  parseEnabledBoardTeamIds,
  resolveEnabledBoardTeamIds,
} from "@/lib/project-boards";

async function loadProjectBoardContext(projectId: string) {
  const project = await prisma.project.findFirst({
    where: { OR: [{ id: projectId }, { slug: projectId }] },
    include: {
      team: { select: { departmentId: true } },
      members: {
        include: {
          user: {
            select: {
              name: true,
              teamId: true,
              memberships: {
                where: { isActive: true },
                select: { team: { select: { id: true } } },
                take: 1,
              },
            },
          },
        },
      },
    },
  });
  if (!project) return null;

  const projectDeptId = project.departmentId ?? project.team?.departmentId ?? null;

  const [ticketTeams, departmentTeams] = await Promise.all([
    prisma.ticket.findMany({
      where: { projectId: project.id, deletedAt: null, parentId: null },
      select: { teamId: true },
      distinct: ["teamId"],
    }),
    projectDeptId
      ? prisma.team.findMany({
          where: { departmentId: projectDeptId },
          select: { id: true },
          orderBy: { name: "asc" },
        })
      : Promise.resolve([]),
  ]);

  const ticketTeamIds = ticketTeams.map((t) => t.teamId);
  const departmentTeamIds = departmentTeams.map((t) => t.id);
  const memberTeamIds = memberTeamIdsFromProject(project.members);
  const addableTeamIds = [...new Set([...departmentTeamIds, ...memberTeamIds])];
  const stored = parseEnabledBoardTeamIds(project.enabledBoardTeamIds);
  const enabled = resolveEnabledBoardTeamIds({
    stored,
    departmentTeamIds,
    ticketTeamIds,
    projectTeamId: project.teamId,
  });

  return { project, departmentTeamIds, memberTeamIds, addableTeamIds, ticketTeamIds, stored, enabled };
}

export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> },
) {
  const { profile, error } = await requireAuth();
  if (error) return error;

  if (!canManageProjectBoards(profile)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const { id } = await params;
  const body = await request.json();
  const action = body.action as string | undefined;
  const teamId = (body.teamId as string | undefined)?.trim();

  if (!action || !teamId) {
    return NextResponse.json({ error: "action and teamId are required" }, { status: 400 });
  }

  const ctx = await loadProjectBoardContext(id);
  if (!ctx) return NextResponse.json({ error: "Not found" }, { status: 404 });

  if (!(await projectInScope(profile, ctx.project.id))) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const { project, addableTeamIds, enabled } = ctx;

  if (action === "add") {
    if (!addableTeamIds.includes(teamId)) {
      return NextResponse.json(
        { error: "Team is not available for this project" },
        { status: 400 },
      );
    }
    if (enabled.includes(teamId)) {
      return NextResponse.json({ enabledBoardTeamIds: enabled });
    }

    const next = [...enabled, teamId].sort();
    await prisma.project.update({
      where: { id: project.id },
      data: { enabledBoardTeamIds: next },
    });
    await broadcastProjectBoardsChange(project.id);
    return NextResponse.json({ enabledBoardTeamIds: next });
  }

  if (action === "remove") {
    const ticketCount = await prisma.ticket.count({
      where: { projectId: project.id, teamId, deletedAt: null },
    });
    if (ticketCount > 0) {
      return NextResponse.json(
        { error: "Cannot remove a board that still has tickets" },
        { status: 409 },
      );
    }

    const next = enabled.filter((tid) => tid !== teamId);
    await prisma.project.update({
      where: { id: project.id },
      data: { enabledBoardTeamIds: next },
    });
    await broadcastProjectBoardsChange(project.id);
    return NextResponse.json({ enabledBoardTeamIds: next });
  }

  return NextResponse.json({ error: "Invalid action" }, { status: 400 });
}
