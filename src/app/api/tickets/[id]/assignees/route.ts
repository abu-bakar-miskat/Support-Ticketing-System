...
export async function POST(
  ...
  // Only act on actual new assignments (not re-adds of existing co-assignees)
  ...
  if (!existing) {
    ...
    // Emit CO_ASSIGNEE_ADDED event — writes to ActivityLog and broadcasts in real-time
    await appendTicketEvent(ticketId, profile.id, "CO_ASSIGNEE_ADDED", {
      userId,
      userName: assignee?.name ?? userId,
    });

    // Drafts may store assignees but must not notify until published.
    if (!ticket.isDraft) {
...
export async function DELETE(
  ...
  // Emit CO_ASSIGNEE_REMOVED event — real-time broadcast to all ticket viewers
  appendTicketEvent(ticketId, profile.id, "CO_ASSIGNEE_REMOVED", {
    userId,
    userName: assignee?.name ?? userId,
  }).catch(() => undefined);
