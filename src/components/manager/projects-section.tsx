  ...
  return (
    <div className="flex flex-col gap-1.5 border-b border-sts-card-border/40 px-4 py-3 last:border-b-0">
      <div className="flex items-center gap-2">
        ...
        <span className="min-w-0 flex-1 truncate font-sans text-[12.5px] font-semibold text-sts-foreground">{p.name}</span>
        <span className="shrink-0 font-mono text-[11px] tabular-nums text-sts-muted">
          {p.done}<span className="text-sts-subtle">/{p.total}</span>
        </span>
        <span className="w-[38px] shrink-0 text-right font-mono text-[11px] font-semibold tabular-nums" style={{ color: barColor }}>
          {pct}%
        </span>
      </div>
      <div className="h-[5px] overflow-hidden rounded-full bg-sts-surface">
        <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: barColor }} />
      </div>
      <div className="flex items-center gap-3">
        {p.overdue > 0 && <span className="font-sans text-[10.5px] text-red-500">{p.overdue} overdue</span>}
        {p.active > 0 && <span className="font-sans text-[10.5px] text-sts-blue">{p.active} in progress</span>}
...
export function ProjectsSection({ projects }: { projects: ProjectHealth[] }) {
  ...
  return (
    <RailCard
      ...
      title="Projects"
      defaultOpen={false}
      aside={<span className="font-sans text-[11px] text-sts-subtle">{projects.length} in scope</span>}
    >
