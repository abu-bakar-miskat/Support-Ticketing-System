...
function PersonCell({ name, avatarUrl, subDepartment }: { name: string; avatarUrl?: string | null; subDepartment?: string }) {
  return (
    <div className="flex min-w-0 items-center gap-1.5">
      ...
      <span className="truncate whitespace-nowrap font-sans text-[11.5px] text-sts-foreground" title={name}>{name.length > 10 ? name.slice(0, 10) + "…" : name}</span>
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <div className="flex h-4 items-center">
          <span className="font-mono text-[11.5px] font-semibold text-sts-foreground">{task.humanId}</span>
          {isRunning && (
        ...
        {/* Title */}
        <p className="font-sans text-[12.5px] font-semibold leading-[18px] text-sts-foreground">{task.title}</p>
        ...
        {task.creatorName && (
          <div className="flex items-center gap-1">
            ...
            <span className="font-sans text-[11.5px] text-sts-subtle">{task.creatorName}</span>
          </div>
        )}
        ...
        <div className="flex h-5 items-center gap-1.5">
          {task.project && (
            <>
              ...
              <span className="font-sans text-[11.5px] text-sts-subtle truncate max-w-[80px]">{task.project}</span>
            </>
          )}
          <CardModuleSegment
          ...
          {task.comments > 0 && (
            <>
              ...
              <span className="font-sans text-[11.5px] text-sts-subtle">{task.comments}</span>
            </>
          )}
          <span className="w-1" />
          <div className="flex items-center -space-x-1">
            {task.assigneeName ? (
              <UserAvatar name={task.assigneeName} avatarUrl={task.assigneeAvatarUrl} userId={task.assigneeId} size={18} className="ring-1 ring-sts-card" meta={{}} />
            ) : (
            ...
            {(task.coAssignees ?? []).slice(0, 2).map((a) => (
              <UserAvatar key={a.id} name={a.name} avatarUrl={a.avatarUrl} userId={a.id} size={18} className="ring-1 ring-sts-card" meta={{}} />
            ))}
            {(task.coAssignees ?? []).length > 2 && (
              <span className="flex size-[18px] shrink-0 items-center justify-center rounded-full bg-sts-surface font-sans text-[11.5px] text-sts-subtle ring-1 ring-sts-card">+{task.coAssignees.length - 2}</span>
            )}
          </div>
        </div>
        ...
        {task.labels.length > 0 && (
          <div className="flex flex-wrap gap-1">
            ...
            {task.labels.length > 3 && (
              <span className="font-sans text-[10px] text-sts-subtle">+{task.labels.length - 3}</span>
            )}
          </div>
        )}
        ...
        <div className="flex h-4 items-center gap-[5px]">
          ...
          {task.startDate && (
            <>
              <span className="font-mono text-[11.5px] font-medium text-sts-muted">{task.startDate}</span>
              <span className="font-mono text-[11.5px] text-sts-subtle">→</span>
            </>
          )}
          <span className={cn(
            "font-mono text-[11.5px] font-medium",
            task.due === "Complete" ? "text-sts-green"
              : task.dueOverdue ? "text-sts-red"
              : task.dueUrgent ? "text-amber-500"
              : task.due ? "text-sts-muted"
              : "text-sts-subtle",
          )}>
      ...
      {subTotal > 0 && expanded && (
        <div className="mx-3 mb-2 border-t border-sts-card-border pt-1">
          {subTicketCards.map((sub) => {
            ...
            return (
              <DrawerLink key={sub.dbId} ticketId={sub.dbId} href={`/tickets/${sub.dbId}`} className="flex h-[26px] items-center gap-2 rounded px-1.5 hover:bg-sts-surface">
...
function ReviewTaskRow({ task, colorMap }: { task: BoardCardData; colorMap: Record<string, string> }) {
  ...
  return (
    <tr className="group border-b border-[#f0f4f8] transition-colors hover:bg-sts-blue/[0.03] dark:border-[#3a3a37] dark:hover:bg-sts-blue/[0.06]">
      <td className={cn("w-[72px] pl-4", LIST_TD)}>
        <DrawerLink ticketId={task.dbId} href={`/tickets/${task.dbId}`} card={task} className="font-mono text-[11.5px] font-semibold text-sts-id hover:underline">{task.humanId}</DrawerLink>
      </td>
      <td className={cn(TITLE_CELL_CLASS, LIST_TD)}>
        <div className={TITLE_INNER_CLASS}>
          {isPR
            ...
            : <Eye className="size-[13px] shrink-0 text-sts-blue" strokeWidth={2} />
          }
          <DrawerLink ticketId={task.dbId} href={`/tickets/${task.dbId}`} card={task} className="flex min-w-0 flex-1 items-center gap-1.5">
            <span className={cn("block size-[7px] shrink-0 rounded-full", pulseCritical && "sts-critical-breathe")} style={{ backgroundColor: priorityColor }} />
            <span className="min-w-0 truncate font-sans text-[13px] text-sts-foreground group-hover:text-sts-blue" title={task.title}>{task.title}</span>
        ...
        {task.project ? (
          <div className="flex min-w-0 items-center gap-2" title={task.project}>
            <ProjectAvatar
              ...
              size={20}
            />
            <span className="min-w-0 truncate font-sans text-[11.5px] text-sts-muted">{task.project}</span>
          </div>
        ) : (
    ...
    <>
      <tr className="group border-b border-[#f0f4f8] transition-colors hover:bg-sts-bg dark:border-[#3a3a37]">
        ...
        <td className={cn(TITLE_CELL_CLASS, LIST_TD)}>
          <div className={TITLE_INNER_CLASS}>
            {hasChildren && (
              <button type="button" onClick={() => setExpanded((v) => !v)} className="flex shrink-0 items-center justify-center text-sts-muted hover:text-sts-foreground">
                <ChevronRight className={cn("size-[13px] transition-transform", expanded && "rotate-90")} />
              </button>
            )}
            <DrawerLink ticketId={task.dbId} href={`/tickets/${task.dbId}`} card={task} className="flex min-w-0 flex-1 items-center gap-1.5">
              <span className={cn("block size-[7px] shrink-0 rounded-full", pulseCritical && "sts-critical-breathe")} style={{ backgroundColor: priorityColor }} />
              <span className="min-w-0 truncate font-sans text-[13px] text-sts-foreground group-hover:text-sts-id" title={task.title}>{truncateTitle(task.title)}</span>
              <TaskListLabels labels={task.labels} />
              {hasChildren && (
                <span className="shrink-0 rounded-full bg-sts-surface px-1.5 py-px font-sans text-[11.5px] text-sts-subtle">{task.subTicketCards.length}</span>
              )}
            </DrawerLink>
          </div>
        </td>
        ...
        <td className={cn(COL_CREATOR, "overflow-hidden", LIST_TD)}>
          {task.creatorName ? <PersonCell name={task.creatorName} avatarUrl={task.creatorAvatarUrl} /> : <span className="font-sans text-[11.5px] text-sts-subtle">—</span>}
        </td>
        <td className={cn(COL_ASSIGNEE, "overflow-hidden", LIST_TD)}>
          <InlineAssigneePicker members={members} currentId={liveAssigneeId} onSelect={handleAssigneeChange}>
            {({ ref, onClick }) => (
              <button ref={ref} type="button" onClick={(e) => { e.stopPropagation(); onClick(); }} className="rounded px-1 py-0.5 transition-colors hover:bg-sts-surface">
                <AssigneeAvatars
        ...
        <td className={cn(COL_PROJECT, "overflow-hidden", LIST_TD)}>
          <div className="flex min-w-0 items-center gap-2" title={task.project}>
            <ProjectAvatar
              ...
              size={22}
            />
            <span className="min-w-0 truncate whitespace-nowrap font-sans text-[11.5px] text-sts-muted">{task.project}</span>
          </div>
        </td>
        ...
        return (
          <tr key={sub.dbId} className="border-b border-[#f0f4f8] bg-sts-bg transition-colors hover:bg-sts-surface dark:border-[#3a3a37]">
            <td className="w-[80px] py-2 pl-4">
              <DrawerLink ticketId={sub.dbId} href={`/tickets/${sub.dbId}`} className="font-mono text-[11.5px] font-semibold text-sts-id hover:underline">{sub.humanId}</DrawerLink>
            </td>
            <td className="max-w-0 py-2 pr-3 pl-5">
              <DrawerLink ticketId={sub.dbId} href={`/tickets/${sub.dbId}`} className="flex min-w-0 items-center gap-2">
                <span className={cn("block size-[6px] shrink-0 rounded-full", subPulseCritical && "sts-critical-breathe")} style={{ backgroundColor: subPriorityColor }} />
                <span className="truncate font-sans text-[12px] text-sts-foreground">{sub.title}</span>
              </DrawerLink>
            </td>
            <td className={cn(COL_STATUS, "py-2")}>
              <span className="font-sans text-[11.5px] text-sts-muted">{sub.status}</span>
            </td>
            ...
            <td className={cn(COL_ASSIGNEE, "py-2")}>
              {sub.assigneeName ? (
                ...
                <span className="block size-5 shrink-0 rounded-full border border-dashed border-sts-subtle" title="Unassigned" />
              )}
            </td>
...
function SubtaskListRow({ task, colorMap }: { task: AssignedSubtask; colorMap: Record<string, string> }) {
  ...
  return (
    <tr className="group border-b border-[#f0f4f8] bg-sts-bg/40 transition-colors hover:bg-sts-bg dark:border-[#3a3a37] dark:bg-sts-surface/20">
      <td className="w-[80px] py-2.5 pl-4">
        <DrawerLink ticketId={task.dbId} href={`/tickets/${task.dbId}`} card={task} className="font-mono text-[11.5px] font-semibold text-sts-id hover:underline">
          {task.humanId}
        </DrawerLink>
      </td>
      <td className={cn(TITLE_CELL_CLASS, "py-2.5")}>
        <DrawerLink ticketId={task.dbId} href={`/tickets/${task.dbId}`} card={task} className="flex min-w-0 items-center gap-2 overflow-hidden">
          <span className={cn("block size-[7px] shrink-0 rounded-full", pulseCritical && "sts-critical-breathe")} style={{ backgroundColor: priorityColor }} />
          <span className="shrink-0 rounded-full border border-sts-card-border bg-sts-surface px-1.5 py-px font-sans text-[10px] font-semibold text-sts-subtle">
            ↳ sub
          </span>
          <span className="truncate font-sans text-[13px] text-sts-foreground group-hover:text-sts-id">{task.title}</span>
        </DrawerLink>
      </td>
      <td className={cn(COL_STATUS, "overflow-hidden", LIST_TD)}>
        <InlineStatusPicker subDepartmentId={task.subDepartmentId} statuses={statuses} current={liveStatus} onSelect={handleStatusChange}>
      ...
      <td className={cn(COL_CREATOR, "overflow-hidden py-2.5")}>
        <Link href={`/tickets/${task.parentDbId}`} className="font-mono text-[11.5px] font-semibold text-sts-id hover:underline">
          {task.parentHumanId}
        </Link>
      </td>
      ...
      <td className={cn(COL_PROJECT, "overflow-hidden py-2.5")}>
        <div className="flex min-w-0 items-center gap-2" title={task.project}>
          <ProjectAvatar
            ...
            size={22}
          />
          <span className="min-w-0 truncate font-sans text-[11.5px] text-sts-muted max-w-[110px]">{task.project}</span>
        </div>
      </td>
...
function BoardColumn({
  ...
  return (
    <div
      ...
      className={cn(
        "flex h-full w-[280px] shrink-0 flex-col gap-2 rounded-[10px] bg-sts-surface p-3 transition-colors sm:w-[300px] lg:w-[min(320px,calc((100cqw-3.5rem)/5))]",
    ...
    >
      <div className="flex h-6 shrink-0 items-center gap-2">
        ...
        <span className="font-sans text-[12px] font-semibold text-sts-foreground">{status.label}</span>
        <span
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
        ...
        {cards.length === 0 && (
          <p className={cn("py-2 text-center font-sans text-[11.5px] transition-colors", isActive ? "text-sts-blue" : "text-sts-subtle")}>
...
export function MyTasksPage({ tasks: initialTasks, subDepartmentStatuses, subDepartmentStatusMap = {}, reviewTasks = [], isManager = false, subtasks = [], hideTitleBar = false }: Props) {
  ...
  return (
    <BoardDndProvider>
      ...
      >
        {!hideTitleBar && (
          <PageHeader title="My Tasks" icon={ListTodo} iconClassName="text-sts-blue" className="mb-3" />
        )}
        ...
        >
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] max-xl:-mx-4 max-xl:px-4 sm:max-xl:-mx-6 sm:max-xl:px-6 xl:overflow-visible xl:px-0 [&::-webkit-scrollbar]:hidden">
            <p className="shrink-0 font-sans text-[12px] text-sts-muted sm:text-[12.5px]">
              <span className="font-semibold text-sts-foreground">{filteredTasks.length}</span>
              {" of "}
              <span className="font-semibold text-sts-foreground">{localTasks.length}</span>
              {" task"}{localTasks.length === 1 ? "" : "s"}
              {totalSubtickets > 0 && (
                <>
                  {" + "}
                  <span className="font-semibold text-sts-foreground">{filteredSubtickets}</span>
                  {filteredSubtickets !== totalSubtickets && (
                    <>
                      {" of "}
                      <span className="font-semibold text-sts-foreground">{totalSubtickets}</span>
                    </>
                  )}
            ...
            {activeExtraFilters > 0 && (
              <button
                ...
                className="flex shrink-0 items-center gap-1 rounded-full border border-sts-card-border bg-sts-surface px-2.5 py-1 font-sans text-[11.5px] whitespace-nowrap text-sts-subtle hover:text-sts-foreground"
              >
                <X className="size-3" />
                Clear
              </button>
          ...
          <div className="h-full min-h-0 overflow-x-auto overflow-y-auto">
            ...
            <table className={MY_TASKS_TABLE_CLASS}>
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="py-20 text-center font-sans text-[13px] text-sts-subtle">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex min-w-[540px] items-center justify-between border-t border-sts-card-border bg-sts-card px-4 py-2.5">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={page === 1}
                    className="flex h-7 w-7 items-center justify-center rounded-md border border-sts-card-border font-sans text-sts-muted transition-colors hover:bg-sts-surface hover:text-sts-foreground disabled:cursor-not-allowed disabled:opacity-40"
                  >
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, i) =>
                      p === "…" ? (
                        <span key={`ellipsis-${i}`} className="flex h-7 w-7 items-center justify-center font-sans text-[11.5px] text-sts-subtle">…</span>
                      ) : (
                        <button
                          key={p}
                          type="button"
                          ...
                          className={cn(
                            ...
                            page === p
                              ...
                              : "border border-sts-card-border text-sts-muted hover:bg-sts-surface hover:text-sts-foreground",
                          )}
                        >
                          {p}
                        </button>
            ...
            {isManager && reviewTasks.length > 0 && (
              <div className="border-t border-sts-card-border">
                <button
                  type="button"
                  onClick={() => setReviewOpen((v) => !v)}
                  aria-expanded={reviewOpen}
                  className="flex w-full items-center gap-2 bg-sts-card px-4 py-2.5 text-left transition-colors hover:bg-sts-surface/50"
                >
                  <ChevronRight
                    className={cn(
                  ...
                  />
                  <Eye className="size-3.5 shrink-0 text-sts-blue" strokeWidth={2} />
                  <span className="font-sans text-[11.5px] font-semibold uppercase tracking-[1.2px] text-sts-muted max-xl:normal-case max-xl:tracking-normal">
                    Team review &amp; pull requests
                  </span>
                  <span className="flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-sts-blue/10 px-1.5 font-sans text-[11.5px] font-bold text-sts-blue">
                    {reviewTasks.length}
                  </span>
                  {!reviewOpen && (
                    <span className="truncate font-sans text-[11px] text-sts-subtle max-sm:hidden">
                    ...
                    <tbody>
                      {reviewGroups.map((g) => (
                        <React.Fragment key={g.name}>
                          <tr className="bg-sts-surface/50">
                            <td colSpan={6} className="px-4 py-1.5">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="flex h-[16px] min-w-[16px] items-center justify-center rounded-full bg-sts-card px-1.5 font-mono text-[10.5px] font-semibold tabular-nums text-sts-muted">
                                  {g.tasks.length}
                                </span>
                              </div>
                            </td>
