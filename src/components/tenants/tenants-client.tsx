...
import { TENANT_TYPES, DEFAULT_TENANT_TYPE, tenantTypeLabel } from "@/lib/tenant-types"
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      >
        ...
        />
        <Select value={type} onValueChange={(v) => setType(v ?? DEFAULT_TENANT_TYPE)}>
          <SelectTrigger className="h-9 min-w-[130px]">
            <span className="font-sans text-[12.5px]">{tenantTypeLabel(type)}</span>
          </SelectTrigger>
          <SelectContent>
            {TENANT_TYPES.map((t) => (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 font-sans text-[11.5px] text-pen-muted">
                    ...
                    <span>
                      {t.departments} dept{t.departments === 1 ? "" : "s"} · {t.members} member
                      {t.members === 1 ? "" : "s"}
                    </span>
                  </div>
                </div>
